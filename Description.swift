/*  The 'Ultimate' Collection View Boilerplate  ∙  1st commit Apr. 03, 2019  ∙  Created by Garth Snyder a.k.a. gladiusKatana ⚔️
 
 
        ______________________________________
       | □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ |    |
       | □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ |    |
       | ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ |    |
       | ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ |    |
       | □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ |  ⃝ |
       | ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ |    |
       | □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ |    |
       | ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ |    |
       | □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ □ ■ |    |
        ⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻⎻
 
 
                                                The 'Ultimate' Collection View Boilerplate
 

                                                is a mini-app you can download and run,
 
 built on a single class (*) that covers most ways you'd probably want a collection view to behave.
 
 Just initialize the collection views you need, with the parameters you need, and add a navigation button for each; that's it.
 
 
 
                                                It's designed to:
 
 
 □■  Save developers effort (**) setting up views, especially custom layouts (depending on the application, these can be tricky.)
 
 □■  Make collection views look, to the developer, more like a simple data structure (like a dictionary)
     rather than the awkward, fragile creatures they often are. (***)
 
 
 
 
 
                        Notes
 
 
 (*) Ok, it's really two: the layout and the collection view controller. But the former is a property of the latter, so it's as if it's one.
 
 (**) In particular, devs who prefer to write most projects purely programmatically, rather than using storyboards.
 
 (***) Try copying & pasting the below Creature right under where he currently is. Does it work for you? Should it?

 
 

 
 
                                                                            ( ◉ )______________
                                  "rather than ///\/{   }//\v\/\//v/\\//\/\/      ▽▽▾▿▿▾▽▽▿▽▽▿▽▾
                                the awkward,   \|\/\\//\\/\/\/\\/\/ \/\\//v \/\\
                            fragile creatures  \//v/\/\ \/\/\//\//\///\//\/\/\\ _⧍⧍▵⧍_▲▵▵▴_▵▴▵
                          they often are" //\//\/ {  \/ / \/\/ \\/\\      }
                            //\\\//\//             {     }          (    }
                                                    {    )           {   )
                                                    {   }             \||\
                                                     /||\
 
 
 
 

 



 
 
                        Coming Updates
  

    [Performance]
  
    faster (& 100% reliable) navigation between collection views, even on older devices
 
 
 
 
    [UI]
  
    cell cornerRadius init parameter
 
 
 
 
    [UX - code]
  
    more explanatory comments
 
 */
 


//A mini-app you can download and run, and extend for your iOS projects.  Built on flexible custom layout & view controller classes, that cover most ways you'd probably want a collection view to behave ⚔️


